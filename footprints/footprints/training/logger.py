...
def log(writer, inputs, outputs, losses, lr, step):
    ...
    # Check if this is the ground dataset (only has image and mask)
    is_ground_dataset = 'mask' in inputs and 'depth' not in inputs

    batch_size = inputs['image'].shape[0]
    num_log = min(4, batch_size)
    for i in range(num_log):
        writer.add_image('image/{}'.format(i), inputs['image'][i], step)

        if not is_ground_dataset:
            ...
            # Ground dataset - just log prediction mask
            if ('all_ground', '1/1') in outputs and i < len(inputs['mask']):
                mask_target = inputs['mask'][i]
                if mask_target.dim() == 2:
                    mask_target = mask_target.unsqueeze(0)
                writer.add_image('target_mask/{}'.format(i),
                                normalise_image(mask_target), step)

                pred_mask = outputs[('all_ground', '1/1')][i]
                if pred_mask.dim() == 2:
                    pred_mask = pred_mask.unsqueeze(0)
                writer.add_image('pred_ground_all/{}'.format(i),
                                normalise_image(pred_mask), step)
