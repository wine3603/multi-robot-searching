...
class LossManager:
    ...
    def __call__(self, predictions, targets):
        ...
        # Check if this is our ground dataset (only has 'image' and 'mask')
        if 'mask' in targets:
            # Our ground dataset - only train for all ground segmentation, ignore depth
            from torch.nn import Sigmoid
            sigmoid = Sigmoid()
            mask = targets['mask']

            for scale_key, output in predictions.items():
                # For our task, just use the all ground channel (second channel, index 1)
                pred = output[:, 1]
                # Squeeze mask to match pred shape - mask is [B, 1, H, W], pred is [B, H, W]
                mask = mask.squeeze(1)
                # We have binary segmentation, use BCE loss with pos_weight for class imbalance
                # Note: pred is logits before sigmoid
                loss = self.bce_loss(pred, mask)
                losses[('all_ground', scale_key)] = loss
                outputs[('all_ground', scale_key)] = sigmoid(pred)
                losses[('loss', scale_key)] = losses[('all_ground', scale_key)]
                all_scale_loss += losses[('loss', scale_key)]

            all_scale_loss /= len(predictions)  # mean over scales
            losses['loss'] = all_scale_loss
            predictions.update(outputs)
            return losses

        # Original behavior for KITTI/Matterport with depth
