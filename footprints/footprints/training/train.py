...
class TrainManager:
    ...
    def create_dataloaders(self):
        ...
        if dataset == 'ground':
            # Ground dataset - already does 80/20 split internally
            # data_dir is directly where images/ and masks/ are
            raw_data_path = self.config[dataset]['dataset']
            train_dataset = dataset_class(raw_data_path, split='train')
            val_dataset = dataset_class(raw_data_path, split='val')
        else:
            # Original logic for KITTI/Matterport
            raw_data_path = self.config[dataset]['dataset']
